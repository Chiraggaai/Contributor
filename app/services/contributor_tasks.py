...
from app.db.persistence import FullState, get_db_path, load_state, save_state
from app.db.revive import revive_all
from app.db.seed import build_seed_state, ensure_seeded_database
...
class ContributorTaskService:
    def __init__(self, *, use_temp_db: bool | None = None) -> None:
...
_service: ContributorTaskService | None = None


def get_contributor_task_service() -> ContributorTaskService:
    global _service
    if _service is None:
        _service = ContributorTaskService()
    return _service


def reset_task_service_singleton() -> None:
    """Call after replacing contributor_tasks.sqlite so the next request loads fresh state."""
    global _service
    _service = None
