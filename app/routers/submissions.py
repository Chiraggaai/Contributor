...
from app.services.submission_store import file_refs_for_ids, store

router = APIRouter(prefix="/api/contributor", tags=["contributor-submissions"])
...
def create_submission(task_id: str, body: CreateSubmissionBody):
...
def patch_submission(submission_id: str, body: PatchSubmissionBody):
    rec = store.get(submission_id)
    if rec is None:
        raise HTTPException(status_code=404, detail="Submission not found")
    if _patch_mutates(body) and not store.is_editable(rec):
        raise HTTPException(
            status_code=403,
            detail=(
...
def submit_submission(submission_id: str, body: SubmitBody):
    rec = store.submit(
        submission_id,
        notes=body.notes,
...
def resubmit_submission(submission_id: str, body: ResubmitBody):
    rec = store.resubmit(
        submission_id,
        notes=body.notes,
        file_ids=body.file_ids,
...
def latest_submission(task_id: str):
    rec = store.latest_for_task(task_id)
    if rec is None:
...
def review_feedback(task_id: str):
    rec = store.latest_for_task(task_id)
    if rec is None:
